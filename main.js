...
const Gameboard = {
  ...
  isBoardEmpty: function () {
    return this.gameBoard.some((val) => {
      return val === "";
    });
  },
    ...
    if (ev.target.textContent === "") {
      if (this.computer.marker === "O") {
        if (this.gameBoard[0] === "X" && this.gameBoard[1] === "X" && this.gameBoard[2] === "") {
          ...
          while (this.isBoardEmpty()) {
            let choice = Math.floor(Math.random() * 9);
            if (this.gameBoard[choice] === "") {
    ...
    if (this.playerVsComputerClicked === true) {
      if (this.player.marker === "X") {
        if (
          ...
          this.gameBoard.every(function (element) {
            return element !== "";
          })
        ) {
  ...
  playerVsComputer: function () {
    this.playerVsComputerButton.addEventListener(
      "click",
      function (ev) {
        if (this.playerVsPlayerClicked === false) {
          ...
          if (this.playerVsComputerClicked === true) {
            this.xSelector.addEventListener(
              "click",
              function (ev) {
          ...
          if (this.playerVsComputerClicked === true && this.markerSelector === false) {
            this.oSelector.addEventListener(
              "click",
              function (ev) {
                if (this.markerSelector === false) {
    ...
    this.xSelector.addEventListener(
      ...
      function () {
        if (this.playerVsPlayerClicked === false) {
          this.xSelector.classList.add("fade-out");
          this.xSelector.addEventListener("animationend", function (ev) {
            ev.target.classList.add("remove");
          });
          this.oSelector.classList.add("fade-out");
          this.oSelector.addEventListener("animationend", function (ev) {
            ev.target.classList.add("remove");
          });
        }
      }.bind(this)
  ...
  playerVsPlayer: function () {
    this.playerVsPlayerButton.addEventListener(
      "click",
      function (ev) {
